...
def detect_breakout(df: pd.DataFrame) -> tuple[str, str]:
    ...
    high55 = float(close.iloc[-56:-1].max()) if len(close) > 56 else high20
    sma20 = float(close.iloc[-20:].mean())
    sma50 = float(close.iloc[-50:].mean()) if len(close) >= 50 else sma20
    vz = volume_zscore(vol)

    if last > high20 * 1.002 and last >= high55 * 0.995 and vz >= 1.0 and last > sma20:
        return "confirmed", "Cleared 20D high on elevated volume — trend breakout"
    if last > high20 * 0.995 and last > sma20 and sma20 >= sma50 * 0.995 and vz >= 0.5:
        return "forming", "Testing breakout shelf / coiling under prior high"
    if last < sma20 and last < sma50:
        return "none", "Below key averages — no breakout structure"
    return "none", "Range / no clean breakout yet"
...
def forecast_7d(df: pd.DataFrame, breakout: str, fii_net: float) -> dict[str, Any]:
    ...
    score = 0.0
    score += float(np.clip(mom5 * 100, -3, 3))
    score += float(np.clip(mom20 * 40, -3, 3))
    score += float(np.clip(vz * 0.8, -2.5, 2.5))
    ...
    # map score to probability 52–85
    prob = int(np.clip(52 + abs(score) * 8, 52, 85))

    # expected band from realized vol * sqrt(7) with directional skew
    move = vol * np.sqrt(7)
...
def detect_events(df: pd.DataFrame, top_n: int = 4, symbol: str = "") -> list[dict[str, Any]]:
    ...
    for idx in spikes.index:
        ...
        try:
            ...
            if vz >= 2:
                vol_note = f" Volume was very heavy (+{vz:.1f}σ vs 20D avg), typical of institutional participation or news-driven flow."
            elif vz >= 1:
                vol_note = f" Volume was elevated (+{vz:.1f}σ), supporting a genuine move rather than a thin print."
            else:
                vol_note = " Volume was near normal, so the swing may reflect positioning / gap rather than broad participation."
        except Exception:  # noqa: BLE001
            vol_note = " Volume context unavailable for this session."

        if direction == "raise":
            reason = (
                f"{symbol or 'Stock'} jumped {pct:+.1f}% on {date}. "
                f"Prior 5 sessions were cumulative {prev5:+.1f}%; the following 3 sessions moved {next3:+.1f}%. "
                f"Common drivers for such spikes: earnings/guidance beats, large block/bulk buys, sector rotation, "
                f"or positive corporate actions."
                f"{vol_note}"
            )
        else:
            reason = (
                f"{symbol or 'Stock'} fell {pct:+.1f}% on {date}. "
                f"Prior 5 sessions were cumulative {prev5:+.1f}%; the following 3 sessions moved {next3:+.1f}%. "
                f"Common drivers for such drops: earnings misses, risk-off / FII selling, large sell blocks, "
                f"regulatory headlines, or sector-wide de-rating."
                f"{vol_note}"
            )
        events.append(
            {
...
def forecast_drivers(df: pd.DataFrame, f7: dict[str, Any], flows: dict[str, Any], deals_n: int, deal_value: float) -> list[dict[str, Any]]:
    ...
    close = df["Close"]
    last = float(close.iloc[-1])
    sma20 = float(close.iloc[-20:].mean()) if len(close) >= 20 else last
    sma50 = float(close.iloc[-50:].mean()) if len(close) >= 50 else sma20
    ...
    return [
        ...
        {
            "name": "Distance vs 20D avg",
            "value": f"{dist20:+.1f}%",
            "impact": "bull" if dist20 > 1 else "bear" if dist20 < -1 else "neutral",
            "note": "Stretch above/below the near-term mean",
        },
        {
            "name": "Distance vs 50D avg",
            "value": f"{dist50:+.1f}%",
            "impact": "bull" if dist50 > 1 else "bear" if dist50 < -1 else "neutral",
            "note": "Medium-term trend alignment",
        },
        {
            "name": "Volume z-score (20D)",
            "value": f"{f7['volumeZ']:+.1f}σ",
        ...
        {
            "name": "Model band width",
            "value": f"{f7.get('volatility7d', 0):.1f}%",
            "impact": "neutral",
            "note": "Implied 7D move size from realized volatility",
        },
    ]
...
def build_stock_row(symbol: str, df: pd.DataFrame, info: dict[str, Any], flows: dict[str, Any], universe: list[str], deals_for_symbol: list[dict]) -> dict[str, Any]:
    ...
    for x in df["Close"].iloc[-15:].tolist():
        try:
            ...
            continue
    series_dates = [d.strftime("%d %b") if hasattr(d, "strftime") else str(d)[:10] for d in df.index[-15:]]
    if len(series_dates) > len(series_vals):
        series_dates = series_dates[-len(series_vals) :]
    deal_value = round(sum(d.get("value") or 0 for d in deals_for_symbol), 1)
    return {
        "symbol": symbol,
        ...
        "events": detect_events(df, symbol=symbol),
...
def stock_detail(symbol: str) -> dict[str, Any]:
    ...
    drivers = forecast_drivers(df, {**row["forecast7d"], "volumeZ": row["volumeZ"]}, flows, row["deals"], row["dealValue"])
    rating = row.get("rating3yDetail") or rating_3y_detail(df)
    news = fetchers.fetch_news(symbol, limit=6)
    ...
    thesis = (
        ...
        f"Drivers use momentum, distance to averages, volume intensity, FII cash, and deal prints."
    )
    kpis = [
